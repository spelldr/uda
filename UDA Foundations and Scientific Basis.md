...
## Foundations, Scientific Basis, and Operational Doctrine

UDA is a doctrine‑driven documentation system engineered to minimize cognitive load, maximize findability, and enforce predictable contributor behavior. It is built on cognitive science, information‑seeking research, and schema‑based learning models. UDA treats documentation as engineered infrastructure, not a wiki.
...
- **Extraneous load** is minimized through:
    ...
    - predictable routing
    - rigid templates
    - single‑intent documents
- **Germane load** is supported by keeping conceptual models explicit and separate from procedures.

UDA’s architecture is intentionally designed to reduce decision‑making overhead for both readers and contributors.
...
- Strict routing → no scent confusion
- Templates → consistent scent patterns

UDA optimizes for high scent strength and low foraging cost.
...
- **References** support schemas
- **Troubleshooting** repairs schema failures

Each doc type aligns with a distinct cognitive function.
...
- References are pure lookup tables

UDA applies minimalism as a structural rule, not a stylistic preference.
...
- Predictable depth
- No mixed‑intent categories
- No contributor‑invented folders
- No “misc” buckets

UDA enforces a stable, predictable IA that scales without entropy.
...
- Routing rules prevent drift
- Governance defines contributor behavior
- Starter kits ensure reproducibility

UDA is designed to be self‑maintaining.

---

# 7. The Four Intent‑Bound Doc Types (UDA’s Core)

|Doc Type|Cognitive Function|Scientific Basis|
|---|---|---|
|**Task**|Procedural execution|CLT, Minimalism|
|**Concept**|Schema formation|Schema Theory|
|**Reference**|Lookup retrieval|Information Foraging|
|**Troubleshooting**|Error recovery|Minimalism, CLT|

These four types form the backbone of UDA.

---

# 8. Failure Modes UDA Prevents
...
UDA’s structure is explicitly designed to prevent these systemic failures.

---

# 9. Doctrine Enforcement (How UDA Maintains Itself)
...
These files define the system’s laws.

---

# 10. UDA Lifecycle Model (How Content Lives in the System)
...
UDA’s lifecycle ensures long‑term stability.

---

# 11. What UDA Ultimately Achieves
...
- Domain‑agnostic scalability

UDA is a documentation architecture grounded in science and enforced through doctrine.
